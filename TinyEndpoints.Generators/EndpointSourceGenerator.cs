...
public class EndpointSourceGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        {
            // Fall back to old behavior if no [GenerateEndpoints] class.
            var sb = new StringBuilder();
            sb.AppendLine("// <auto-generated/>");
            sb.AppendLine("using Microsoft.AspNetCore.Builder;");
            sb.AppendLine("namespace TinyEndpoints; internal static class __GeneratedEndpointRegistration {");
            foreach (var kvp in instanceFields)
            {
                sb.AppendLine($"private static readonly {kvp.Key} {kvp.Value} = new {kvp.Key}();");
            }
            ...
            sb.Append(mappingBuilder.ToString());
            sb.AppendLine("} }");
            context.AddSource("GeneratedEndpoints.g.cs", sb.ToString());
        }
    }
